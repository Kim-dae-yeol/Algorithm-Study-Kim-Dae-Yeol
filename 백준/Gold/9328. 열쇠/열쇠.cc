...
void found_key(){
    for(auto e : doors){
        char tile = building[e.first][e.second];
        if(has_key[tile - 'A'] && !vis[e.first][e.second]){
            q.push({e.first, e.second});
        }
    }
}

void bfs(int &result_out){
    while(q.empty() == false){
...
int main(){
    ...
    while(t--){
        ...
        if(key != "0") {
            while(key[idx]){
                if(key[idx] =='0') break;
                has_key[key[idx] - 'a'] = true;
                idx++;
            }
        }
        
        for(auto sp : startPositions){
            vis[sp.first][sp.second] = true;
            q.push(sp);
        }
        found_key();
        bfs(result);
        
        fill(has_key, has_key + 26, false);
        doors.clear();
